...
for ($i = 0; $i -lt $args.Count; $i++) {
    switch ($args[$i]) {
        ...
        "--install-repo" { $InstallRepo = $args[$i + 1]; $i++; continue }
...
# Pass --update-repo to agent so it uses the same repo for self-update
$KomariArgs += "--update-repo"
$KomariArgs += $InstallRepo
...
if ($InstallVersion -ne "") {
    ...
    $ApiUrl = "https://api.github.com/repos/$InstallRepo/releases/latest"
    try {
...
$BinaryName = "komari-agent-windows-$arch.exe"
$DownloadUrl = if ($GitHubProxy) { "$GitHubProxy/https://github.com/$InstallRepo/releases/download/$versionToInstall/$BinaryName" } else { "https://github.com/$InstallRepo/releases/download/$versionToInstall/$BinaryName" }
