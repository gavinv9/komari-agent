...
func rebuildTicker(old *time.Ticker, oldInterval float64, label string) (*time.Ticker, float64) {
	newInterval := math.Max(1, runtimeconfig.ReportInterval())
	if newInterval == oldInterval {
		return old, oldInterval
	}
	log.Printf("%s interval changed from %.1f to %.1f seconds", label, oldInterval, newInterval)
	old.Stop()
	// drain 残留 tick
	select {
	case <-old.C:
	...
	return time.NewTicker(time.Duration(newInterval * float64(time.Second))), newInterval
}
...
func EstablishWebSocketConnection() {
	...
	var err error
	interval := math.Max(1, runtimeconfig.ReportInterval())
	...
	for {
		...
		case <-runtimeconfig.Changes():
			dataTicker, interval = rebuildTicker(dataTicker, interval, "Report")
		case <-dataTicker.C:
			if conn == nil {
...
func runPostFallback(websocketEndpoint string, interval float64) (*ws.SafeConn, error) {
	...
	for {
		select {
		case <-runtimeconfig.Changes():
			reportTicker, interval = rebuildTicker(reportTicker, interval, "POST fallback report")
		case <-reportTicker.C:
...
func runV2PullLoop(ctx context.Context, errCh chan<- error) {
	for {
		...
		payload := v2.NewRequest(pullID, v2.MethodAgentPull, map[string]interface{}{
			"capabilities":  []string{"exec", "ping", "message", "event", "terminal", "route"},
...
func postV2RequestContext(ctx context.Context, payload []byte) (*v2.Response, error) {
	...
	// 通过 header 传递 token，避免 URL 日志泄露
	req.Header.Set(AgentTokenHeader, flags.Token)
	if compressed {
...
func processV2Event(conn *ws.SafeConn, method string, params interface{}, eventID string) bool {
	...
	case v2.MethodAgentConfig:
		...
		if err := v2.BindParams(params, &p); err == nil {
			processRuntimeConfig(p, eventID)
			return true
		} else {
...
func establishTerminalConnection(token, id, endpoint string) {
	endpoint = strings.TrimSuffix(endpoint, "/") + "/api/clients/terminal?id=" + id
