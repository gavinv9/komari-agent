...
var RootCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if flags.MonthRotate < 0 || flags.MonthRotate > 31 {
			return fmt.Errorf("invalid month rotate day %d: expected 0 or a day from 1 to 31", flags.MonthRotate)
		}
		...
		// Apply update repo override: env var > flag > compiled default
		if flags.UpdateRepo != "" {
			if update.IsValidRepoSlug(flags.UpdateRepo) {
				update.Repo = flags.UpdateRepo
			} else {
				log.Printf("[WARNING] Ignoring invalid --update-repo=%q: must match owner/repo", flags.UpdateRepo)
			}
		}
		update.ApplyRepoOverride() // env var AGENT_UPDATE_REPO takes highest priority
...
func init() {
	...
	RootCmd.PersistentFlags().StringVar(&flags.UpdateRepo, "update-repo", "", "Custom update repository slug (owner/repo) for agent self-update")
