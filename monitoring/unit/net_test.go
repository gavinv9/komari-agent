...
func TestNetworkSpeedWithoutMonthRotate(t *testing.T) {
	// 保存并设置 runtimeconfig
	origState := runtimeconfig.Snapshot()
	defer runtimeconfig.Set(origState)

	runtimeconfig.Set(runtimeconfig.State{
		MonthRotate: 0,
		Interval:    origState.Interval,
	})
...
func TestNetworkSpeedWithNicFilters(t *testing.T) {
	...
	state := origState
	state.MonthRotate = 0
	state.ExcludeNics = "lo,docker0"
	runtimeconfig.Set(state)
